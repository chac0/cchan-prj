...
@Inject()
export class ProductReceiptableTemplate implements MailTemplateCreator {
  ...
  public async construct([order]: OrderForMail[]): Promise<MailTemplate['template']> {
    const result = await this.mailTemplateRepository.find(MailTemplateDocument.PRODUCT_RECEIPTABLE);
